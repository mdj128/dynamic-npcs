...
To pin a version, append a tag: `https://github.com/mdj128/dynamic-npcs.git#v0.5.0`

Or add it to `Packages/manifest.json` directly:

```json
"com.mdj.dynamicnpcs": "https://github.com/mdj128/dynamic-npcs.git#v0.5.0"
```
...
| NeuCodec decoder + encoder + NeuTTS backbone, mirrored on this repo's releases | Apache-2.0 | Neuphonic's models; the decoder carries an ONNX graph rewrite for Unity's importer, the encoder and backbone are unmodified. See the [release notes](https://github.com/mdj128/dynamic-npcs/releases/tag/codec-v1) |
...
- **Bake fails with `GatedRepoError: 401`** — baking should need no account; it loads mirrored encoder weights from disk. Delete `Library/DynamicNpcs/bake-venv` and bake again to rebuild the environment. As a fallback, a Hugging Face read token in the setup window works around it.
